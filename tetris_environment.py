...
class Tetris(gym.Env):
            ...
            for idx, i in enumerate(range(int(zero_zero[0]), int(zero_zero[0] + 4))):
                for val_idx, val in enumerate(block[idx]):
                    if field[i][int(val_idx + zero_zero[1])] != (0 and val) and val != 0:
                        possible = False
                    elif possible: field[i][int(val_idx + zero_zero[1])] = val
        except:
            possible = False
    ...
    def render(self, render_mode="human", ms: int = 1000):
        ...
        nbs = []
        nbs.append(self._convert_points_to_array(self.next_blocks[0]))
        nbs.append(self._convert_points_to_array(self.next_blocks[1]))

        self.prev_1.delete("all")
        self.prev_1.create_rectangle(2,2, 30 * 4 * 4, 30 * 4)        
        for idx_col, column in enumerate(nbs[0]):
            for idx_val, val in enumerate(column):
        ...
        self.prev_1.pack(side = "left")
        self.prev_2.pack(side = "right")
